...
int testInit(TTree** tree,  float score) {                   // tests init
	...
	ASSERT((*tree)->nil != NULL, "Init-07");
	ASSERT((*tree)->root == (*tree)->nil, "Init-08");
	ASSERT((*tree)->root == (*tree)->root->lt, "Init-09");
	ASSERT((*tree)->root == (*tree)->root->rt, "Init-10");
	ASSERT((*tree)->root == (*tree)->root->pt, "Init-11");
	ASSERT((*tree)->root == (*tree)->root->next, "Init-12");
	ASSERT((*tree)->root == (*tree)->root->prev, "Init-13");
	ASSERT((*tree)->root == (*tree)->root->end, "Init-14");
	ASSERT((*tree)->size == 0, "Init-15");
...
int testInsert(TTree** tree,  float score) {                   // tests insert
	...
	ASSERT((*tree)->root->lt == (*tree)->nil, "Insert-02");
	ASSERT((*tree)->root->rt == (*tree)->nil, "Insert-03");
...
int testSearch(TTree** tree, float score) {                   // tests contains
	long value = 4;
	ASSERT((search((*tree), (*tree)->root, &value) != (*tree)->nil) == 1,"Search-01");

	value = 10;
	ASSERT((search((*tree), (*tree)->root,&value) != (*tree)->nil) == 0,"Search-02");

	value = 0;
	ASSERT(search((*tree), (*tree)->root,&value) != (*tree)->nil,"Search-03");
...
int testMinMax(TTree* (*tree),  float score) { // test min max
	...
	ASSERT(*((long*)minimum((*tree),(*tree)->root)->elem) == 0l, "MinMax-01");
	ASSERT(minimum((*tree),(*tree)->root)->lt == (*tree)->nil, "MinMax-02");
	ASSERT(*((long*)minimum((*tree), (*tree)->root->rt)->elem) == 4l, "MinMax-03");

	ASSERT(*((long*)maximum((*tree), (*tree)->root)->elem) == 8l, "MinMax-04");
	ASSERT(maximum((*tree), (*tree)->root)->rt == (*tree)->nil, "MinMax-05");
	ASSERT(*((long*)maximum((*tree), (*tree)->root->lt)->elem) == 2l, "MinMax-03");
	printf(". ");
...
int testSuccPred(TTree* (*tree),  float score) { 						// test successor predecessor
	...
	long value;
	ASSERT(successor((*tree), maximum((*tree), (*tree)->root)) == (*tree)->nil,"Successor-01");

	value = 6;
	ASSERT(*((long*)successor((*tree), search((*tree),(*tree)->root,&value))->elem) == 7l,"Successor-02");

	value = 2;
	ASSERT(*((long*)successor((*tree), search((*tree),(*tree)->root,&value))->elem) == 3l,"Successor-03");


	ASSERT(predecessor((*tree), minimum((*tree), (*tree)->root)) == (*tree)->nil,"Predecesor-01");

	value = 5;
	ASSERT(*((long*)predecessor((*tree), search((*tree), (*tree)->root,&value))->elem) == 4l,"Predecesor-02")

	value = 7;
	ASSERT(*((long*)predecessor((*tree), search((*tree), (*tree)->root,&value))->elem) == 6l,"Predecesor-03")
...
int testDelete(TTree **tree, float score) { 						// test delete
	...
	ASSERT((*tree)->root->rt->rt->lt == (*tree)->nil, "Delete-02");
	...
	ASSERT((*tree)->root->rt->lt == (*tree)->nil, "Delete-09");
	...
	// Test empty tree and nil tree links
	value = 2;
	delete((*tree),&value);
	value = 1;
	delete((*tree),&value);
	value = 5;
	delete((*tree),&value);
	ASSERT((*tree)->root == (*tree)->nil, "Delete-19");
	ASSERT((*tree)->root->rt == (*tree)->nil, "Delete-20");
	ASSERT((*tree)->root->lt == (*tree)->nil, "Delete-21");
	ASSERT((*tree)->root->pt == (*tree)->nil, "Delete-22");
	ASSERT((*tree)->root->height == 0, "Delete-23");
...
int testTreeListInsert(TTree **tree, float score) {
	...
	ASSERT(minimum(*tree,(*tree)->root)->prev == (*tree)->nil, "InsertList-01");
	ASSERT(maximum(*tree,(*tree)->root)->end->next == (*tree)->nil, "InsertList-02");
	ASSERT(*((long*)(*tree)->root->elem) == 3, "InsertList-03");
	ASSERT((*tree)->root == (*tree)->root->end, "InsertList-04");
	ASSERT((*tree)->root->prev == predecessor(*tree, (*tree)->root), "InsertList-05");
	ASSERT((*tree)->root->next == successor(*tree, (*tree)->root), "InsertList-06");

	insert(*(tree),values + 3, values + 3);
	ASSERT((*tree)->root != (*tree)->root->end, "InsertList-07");
	ASSERT((*tree)->root->next == (*tree)->root->end, "InsertList-08");
	ASSERT((*tree)->root->prev == predecessor(*tree, (*tree)->root), "InsertList-09");
	ASSERT((*tree)->root->end->next == successor(*tree, (*tree)->root), "InsertList-10");
	...
	ASSERT((*tree)->root->next->next == successor(*tree, (*tree)->root)->prev, "InsertList-14");
	ASSERT((*tree)->root == predecessor(*tree, (*tree)->root)->next, "InsertList-15");

	insert(*(tree),values, values);
	ASSERT( *((long*)minimum((*tree), (*tree)->root)->elem) == 0 , "InsertList-16");
	ASSERT( *((long*)minimum((*tree), (*tree)->root)->next->elem) == 0 , "InsertList-17");
	ASSERT( *((long*)minimum((*tree), (*tree)->root)->end->next->elem) == 1 , "InsertList-18");
	ASSERT(minimum((*tree), (*tree)->root)->prev == (*tree)->nil , "InsertList-19");
	ASSERT(minimum((*tree), (*tree)->root)->prev == (*tree)->nil , "InsertList-20");
	ASSERT(minimum((*tree), (*tree)->root)->next->next == minimum((*tree), (*tree)->root)->end->next , "InsertList-21");


	insert(*(tree),values + 8, values + 8);
	ASSERT( *((long*)maximum((*tree), (*tree)->root)->elem) == 8, "InsertList-22");
	ASSERT( *((long*)maximum((*tree), (*tree)->root)->next->elem) == 8, "InsertList-23");
	ASSERT( *((long*)maximum((*tree), (*tree)->root)->prev->elem) == 7l, "InsertList-24");
	ASSERT(maximum((*tree), (*tree)->root)->end->next == (*tree)->nil , "InsertList-25");
	ASSERT(maximum((*tree), (*tree)->root)->next->next == (*tree)->nil , "InsertList-26");
	ASSERT(maximum((*tree), (*tree)->root)->next->next == maximum((*tree), (*tree)->root)->end->next , "InsertList-27");
...
int testTreeListDelete(TTree **tree, float score) {
	...
	value = 8;
	delete((*tree),&value);
	ASSERT(*((long*)maximum((*tree), (*tree)->root)->elem) == 8, "DeleteList-24");
	ASSERT(*((long*)maximum((*tree), (*tree)->root)->end->elem) == 8, "DeleteList-25");
	ASSERT(*((long*)maximum((*tree), (*tree)->root)->prev->elem) == 7l, "DeleteList-26");
	ASSERT(maximum((*tree), (*tree)->root)->end->next == (*tree)->nil , "DeleteList-27");
	ASSERT(maximum((*tree), (*tree)->root)->next == (*tree)->nil , "DeleteList-28");


	delete((*tree),&value);
	ASSERT(*((long*)maximum((*tree), (*tree)->root)->elem) == 7, "DeleteList-29");
	ASSERT(*((long*)maximum((*tree), (*tree)->root)->end->elem) == 7, "DeleteList-30");
	ASSERT(*((long*)maximum((*tree), (*tree)->root)->prev->elem) == 6l, "DeleteList-31");
	ASSERT(maximum((*tree), (*tree)->root)->end->next == (*tree)->nil , "DeleteList-32");
	ASSERT(maximum((*tree), (*tree)->root)->next == (*tree)->nil , "DeleteList-33");


	value = 0;
	delete((*tree),&value);
	ASSERT(*((long*)minimum((*tree), (*tree)->root)->elem) == 0, "DeleteList-34");
	ASSERT(*((long*)minimum((*tree), (*tree)->root)->end->elem) == 0, "DeleteList-35");
	ASSERT(*((long*)minimum((*tree), (*tree)->root)->next->elem) == 1l, "DeleteList-36");
	ASSERT(minimum((*tree), (*tree)->root)->prev == (*tree)->nil , "DeleteList-37");


	delete((*tree),&value);
	ASSERT(*((long*)minimum((*tree), (*tree)->root)->elem) == 1, "DeleteList-38");
	ASSERT(*((long*)minimum((*tree), (*tree)->root)->end->elem) == 1, "DeleteList-39");
	ASSERT(*((long*)minimum((*tree), (*tree)->root)->next->elem) == 2l, "DeleteList-40");
	ASSERT(minimum((*tree), (*tree)->root)->prev == (*tree)->nil , "DeleteList-41");
...
int main(int argc, char* argv[]) {
	Test tests[] =                                    // Define the tests you want
	{
			...
			{ &testSuccPred, 0.025 },
			{ &testDelete, 0.075 },
			{ &testTreeListInsert, 0.1},
			{ &testTreeListDelete, 0.1},
			{ &testFree, 0.05 },
	};
